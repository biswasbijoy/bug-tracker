...
export default function MobileDrawer({ isOpen, onClose }: MobileDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="relative w-64 h-full bg-[var(--color-surface)] shadow-[var(--shadow-dialog)] flex flex-col animate-slide-in-left">
        <div className="flex items-center justify-between h-12 px-3 border-b border-[var(--color-border)]">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xs font-bold text-[var(--color-text-primary)] leading-tight">SQA Tracker</h1>
              <p className="text-[10px] text-[var(--color-text-muted)] leading-tight">{user?.name}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1 rounded hover:bg-[var(--color-border-light)] text-[var(--color-text-muted)]">
        ...
        <nav className="flex-1 overflow-y-auto p-1.5 space-y-0.5">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <svg className={cn('w-4 h-4 shrink-0', isActive ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
